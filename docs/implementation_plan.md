...
| 维度 | 飞宇智控 (Feiyu) | 千莺云控 (Qianying) | **本项目方案 (Our Solution)** |
| :--- | :--- | :--- | :--- |
| **定位** | 卖硬件模块 (To C/Modder) | 卖加盟/生意模式 (To B) | **自主研发技术底座 (For Developers)** |
| **核心产品** | 4G 遥控模块 (黑盒) | 实体沙盘 + 运营系统 | **全套源码 + ESP32 低成本硬件** |
| **单车成本** | 约 **600-800元** (仅模块) | 加盟费 + 设备昂贵 | 约 **150元** (含车体+改装) |
| **延迟** | 标称低延迟 (用户测约 500ms) | 商业级优化 (未知) | **< 200ms (WebSocket 裸流优化)** |
| **自由度** | 低 (官方给什么用什么) | 低 (受制于平台) | **极高 (代码在手，随意魔改)** |
| **适用场景** | 个人玩家改装单车 | 商业游乐场加盟 | **低成本批量部署 / 二次开发** |

**结论**: 我们的方案在 **“成本”** 和 **“可控性”** 上有绝对优势，适合能够自己动手集成的团队。

### 1.4 深度对比：技术与载具 (Deep Dive)

#### A. 技术架构对比 (Technology)
| 特性 | 飞宇智控 (Feiyu) | 千莺云控 (Qianying) | **本项目 (ESP32)** |
| :--- | :--- | :--- | :--- |
| **通信协议** | P2P UDP / WebRTC (猜测) | 5G/4G 专网协议 | **WebSocket (TCP)** |
| **视频推流** | 硬件编码 (H.264/H.265) | 工业级编码 | **MJPEG (逐帧图片)** |
| **优缺点** | 🟢 节省流量<br>🔴 延迟不稳定 | 🟢在此场景极稳<br>🔴 依赖昂贵基站 | 🟢 **开发极简，兼容所有浏览器**<br>🟡 流量稍大 (MJPEG) |
| **二次开发** | ❌ 封闭系统 (黑盒) | ❌ 需官方授权 SDK | **✅ 100% 开源 (C++/JS/Dart)** |

#### B. 工程车硬件对比 (Vehicle Hardware)
| 特性 | 飞宇智控 | 千莺云控 | **本项目** |
| :--- | :--- | :--- | :--- |
| **载具形态** | **模块化改装** | **整体解决方案** | **原型/DIY 改装** |
| **适用车型** | 任何能插接收机的 RC 车 | 定制液压/重型仿真车 | **汇纳 550/580 等成型玩具** |
| **控制方式** | 模拟 PWM 信号 (伪装成接收机) |CAN总线 / 工业PLC | **直接驱动电机/串口通信** |
| **改装难度** | 低 (插线即用) | 无 (买来即用) | **中 (需焊接/接线/刷固件)** |
| **典型成本** | 600元模块 + 车费 | 10000元+ (整套) | **150元 (板子+电池+辅材)** |

#### C. 体验对比 (User Experience)
| 维度 | 飞宇智控 (Feiyu) | 千莺云控 (Qianying) | **本项目 (ESP32)** |
| :--- | :--- | :--- | :--- |
| **操控感 (Operating)** | **有“漂浮感”**<br>(延迟约 500ms，刹车需预判) | **沉重/仿真**<br>(机械液压本身慢，掩盖了网络延迟) | **“跟手/街机感”**<br>(延迟 < 200ms，指哪打哪) |
| **观看感 (Viewing)** | **高清 (HD)**<br>(720P/1080P，画质细腻) | **高清 (HD)**<br>(大屏沉浸式) | **复古 (240P/QVGA)**<br>(画质一般，但极为流畅) |
| **适用人群** | **风景党**<br>(喜欢慢慢开，看清楚细节) | **职业模友**<br>(追求仿真施工流程) | **竞技/操控党**<br>(追求反应速度和技术验证) |
